...
We use hls4ml to convert trained CNN models into hardware designs. The synthesis flow takes a trained Keras or QKeras model, generates an HLS implementation, validates the generated design with C simulation, and produces latency and resource estimates after synthesis. The final candidates are then evaluated in terms of added latency and added FPGA resources relative to the Coyote baseline.
...
\[
    \text{trained CNN}
    \rightarrow
    \text{hls4ml conversion}
    \rightarrow
    \text{C simulation}
    \rightarrow
    \text{HLS synthesis}
    \rightarrow
    \text{latency and resource report}.
\]
...
Figure~\ref{fig:hls-tradeoff} shows the main latency and resource tradeoff. Each point corresponds to a synthesized model configuration. The relevant deployment points are those with useful F1, acceptable latency, and moderate resource overhead.

\begin{figure}[t]
    \centering
    % Replace with the exported HLS tradeoff plot from the presentation.
    % Recommended file name: figures/hls_latency_resource_f1.pdf
    \includegraphics[width=0.88\linewidth]{figures/hls_latency_resource_f1.pdf}
    \caption{Latency, resource, and F1 tradeoff across synthesized CNN candidates. The final design point must balance detection quality with deployment cost.}
    \label{fig:hls-tradeoff}
\end{figure}

The best software model is not necessarily the best hardware model. For deployment, the useful model is the one that fits into the Coyote system with low enough latency and acceptable area overhead.

\section{Final Synthesized Candidates}
\label{sec:final-synthesized-candidates}

The final comparison focuses on two synthesized models. The 256$\times$7 model is the preferred deployment candidate. The 512$\times$7 model is the accuracy-oriented alternative. Both models use seven convolutional stages, but they differ in input resolution.
...
\begin{table}[t]
    \centering
    \caption{Final synthesized candidates. Added resources are measured relative to the baseline Coyote design.}
...
The 256$\times$7 model is the better deployment candidate. It has lower added latency and much lower BRAM overhead. Its F1 is only two percentage points lower than the 512$\times$7 model, and its original-set TPR is higher. The BRAM difference is the main reason for preferring 256$\times$7. The 512$\times$7 model adds 61.78\% BRAM relative to the baseline Coyote design, while the 256$\times$7 model adds 17.86\%.

\section{Latency Context}
\label{sec:latency-context}

The added inspection latency should be interpreted relative to the latency of partial reconfiguration and serverless invocation. Table~\ref{tab:latency-context} compares the 256$\times$7 and 512$\times$7 inspection latencies with several reference points.
...
\begin{table}[t]
    \centering
    \caption{Baseline Coyote resource usage and added checker overhead.}
...
Quantization and pruning are evaluated as hardware-aware compression techniques. Quantization reduces arithmetic precision. Pruning removes weights and increases sparsity. Both can reduce the cost of a neural network, but the benefit depends on the synthesis flow and on whether the generated hardware can exploit the reduced precision or sparsity.
