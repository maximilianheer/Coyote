...
Figure~\ref{fig:hls-tradeoff} shows the W8A8 P50 resource-strategy sweep for the two selected model families only. It is not the full hls4ml design-space sweep. Circles and squares show reuse-factor sweeps for the 256$\times$7 and 512$\times$7 models. The starred points show the manually tuned v1 candidates used for deployment evaluation. These manual candidates keep the same reported F1 as the corresponding resource-strategy sweep while reducing latency and LUT utilization.
...
\begin{figure}[p]
    ...
    \begin{subfigure}{0.96\linewidth}
        \centering
        % Source image copied from ../ml_deep_bitstream_inspection/hls4ml/results/plots/lut_f1_frontier.png
        \includegraphics[width=\linewidth]{figures/hls_lut_f1_frontier.png}
        \caption{Best achievable F1 as the LUT budget increases.}
        \label{fig:hls-lut-f1-frontier}
    \end{subfigure}
    \caption{Broader hls4ml design-space frontiers. Without per-layer hand optimization, higher F1 tends to require more latency or more LUT utilization.}
    \label{fig:hls-frontiers}
\end{figure}

The best software model is not necessarily the best hardware model. For deployment, the useful model is the one that fits into the Coyote system with low enough latency and acceptable area overhead. The manual v1 points are therefore the relevant candidates for the final Coyote comparison.
