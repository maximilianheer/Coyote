...
\begin{figure}[t]
    \centering
    % Replace with the exported figure from the presentation.
    % Recommended file name: figures/dataset_generation_pipeline.pdf
...
Table~\ref{tab:dataset-composition} summarizes the dataset. The exact counts should be filled in from the final generated dataset. The important distinction is between the original test set and the extended RO-footprint set. The original test set evaluates normal benign versus RO-based malicious classification. The extended set evaluates whether detection improves as the malicious RO footprint grows.

\begin{table}[t]
    \centering
    \caption{Dataset composition. Fill in the final counts after freezing the dataset.}
    \label{tab:dataset-composition}
    \begin{tabular}{p{0.20\linewidth} p{0.28\linewidth} p{0.20\linewidth} r r r}
        \toprule
        \textbf{Class} & \textbf{Source} & \textbf{Variation} & \textbf{Train} & \textbf{Test} & \textbf{Total} \\
        \midrule
        Benign &
        Coyote applications &
        Floorplans and configs &
        TODO & TODO & TODO \\
        \midrule
        Malicious &
        RO payloads &
        RO count and floorplan &
        TODO & TODO & TODO \\
        \midrule
        Extended RO &
