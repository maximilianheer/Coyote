...
The dataset also has possible confounders. Since labels are assigned by construction, the model may learn features correlated with the generation process. These could include payload size, routing density, floorplan effects, or configuration artifacts. The floorplan and configuration sweeps reduce this risk, but they do not eliminate it.
...
Low-footprint attacks deserve special attention. A stronger benchmark should include subtle payloads with different RO densities and placements. This would test whether the classifier can detect malicious structure without relying on large payload size.

Future work should also evaluate other model families. MLPs, decision trees, tiny CNNs, binary neural networks, and FINN-style quantized models may provide better latency and resource tradeoffs. These models should be compared under the same dataset, metrics, and synthesis flow.
